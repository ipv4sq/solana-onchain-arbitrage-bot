...
mod tests {
    ...
    async fn verify_base_to_quote_matches_simulation() -> AResult<()> {
        ...
        println!("  Pool status: {}", config.pool_data.status);
        println!("  Config ID: {}", config.pool_data.amm_config);
        ...
        println!("  Authority: {}", accounts.authority.pubkey);
        println!("  AMM Config: {}", accounts.amm_config.pubkey);
        println!("  Base vault: {}", accounts.input_vault.pubkey);
        println!("  Quote vault: {}", accounts.output_vault.pubkey);
        println!("  User base token: {}", accounts.input_token_account.pubkey);
        println!(
            "  User quote token: {}",
        ...
        let result = simulate_raydium_cpmm_swap_and_get_balance_diff(
            &POOL,
            &payer,
            amount_in,
            min_amount_out,
