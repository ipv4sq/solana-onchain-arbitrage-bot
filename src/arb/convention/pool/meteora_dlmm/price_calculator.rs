...
mod tests {
    ...
    async fn test_pool_sol_usdc_price() {
        ...
        assert!(
            quote_sol_to_usdc.mid_price >= Decimal::from(150) && quote_sol_to_usdc.mid_price <= Decimal::from(250),
            "SOL price outside expected range: {} USDC (expected 150-250 USDC)",
            quote_sol_to_usdc.mid_price
        );
        
        assert!(
            quote_usdc_to_sol.mid_price >= Decimal::new(4, 3) && quote_usdc_to_sol.mid_price <= Decimal::new(67, 4),
    ...
    async fn test_pool_trump_usdc_price() {
        ...
        assert!(
            quote_trump_to_usdc.mid_price >= Decimal::from(5) && quote_trump_to_usdc.mid_price <= Decimal::from(50),
