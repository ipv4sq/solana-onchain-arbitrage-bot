...
mod tests {
    ...
    #[tokio::test]
    async fn test_pool_5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6_sol_usdc_price() {
        ...
        println!("\n=== Testing Pool 5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6 ===");
        println!("This is a USDC/SOL Meteora DLMM pool\n");

        let usdc_mint = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_pubkey();
        let sol_mint = Mints::WSOL;
        
        let _ = MintRecordRepository::upsert_mint(MintRecord {
            address: usdc_mint.into(),
            symbol: "USDC".to_string(),
            decimals: 6,
            program: TokenProgram::SPL_TOKEN.into(),
            created_at: None,
            updated_at: None,
        })
        .await;
        ...
        println!("Pool Data:");
        println!("  Token X Mint: {}", pool_data.token_x_mint);
        println!("  Token Y Mint: {}", pool_data.token_y_mint);
        println!("  Active ID: {}", pool_data.active_id);
        println!("  Bin Step: {}", pool_data.bin_step);
        println!("  Reserve X: {}", pool_data.reserve_x);
        println!("  Reserve Y: {}", pool_data.reserve_y);
        
        let is_usdc_token_x = pool_data.token_x_mint == usdc_mint;
        let is_sol_token_x = pool_data.token_x_mint == sol_mint;
        
        if is_usdc_token_x {
            println!("\nToken X is USDC (6 decimals)");
            println!("Token Y is SOL (9 decimals)");
        } else if is_sol_token_x {
            println!("\nToken X is SOL (9 decimals)");
            println!("Token Y is USDC (6 decimals)");
        }
        
        use rust_decimal::MathematicalOps;
        let base = Decimal::ONE + Decimal::from(pool_data.bin_step) / Decimal::from(10_000u32);
        let px_without_scale = base.powi(pool_data.active_id as i64);
        
        println!("\n=== Debug Price Calculation ===");
        println!("Base (1 + bin_step/10000) = {}", base);
        println!("Active ID = {}", pool_data.active_id);
        println!("Price without decimals: base^active_id = {}", px_without_scale);
        println!("This means: 1 token_x ({}) -> {} token_y ({})", 
                if is_sol_token_x { "SOL" } else { "USDC" },
                px_without_scale,
                if is_sol_token_x { "USDC" } else { "SOL" });
        
        println!("\n=== Testing SOL -> USDC Direction ===");
        println!("From: SOL ({})", sol_mint);
        println!("To: USDC ({})", usdc_mint);
        println!("Direction should be: {}", if pool_data.token_x_mint == sol_mint { "XtoY" } else { "YtoX" });
        ...
        println!("\n=== Testing USDC -> SOL Direction ===");
        println!("From: USDC ({})", usdc_mint);
        println!("To: SOL ({})", sol_mint);
        println!("Direction should be: {}", if pool_data.token_x_mint == usdc_mint { "XtoY" } else { "YtoX" });
        ...
        println!("1 SOL = {} USDC (mid price, 0 slippage)", quote_sol_to_usdc.mid_price);
        println!("1 USDC = {} SOL (mid price, 0 slippage)", quote_usdc_to_sol.mid_price);
        
        let reciprocal_check = quote_sol_to_usdc.mid_price * quote_usdc_to_sol.mid_price;
        println!("\nReciprocal check (should be ~1): {}", reciprocal_check);
        ...
        assert!(
            quote_sol_to_usdc.mid_price > Decimal::from(50) && quote_sol_to_usdc.mid_price < Decimal::from(500),
            "SOL price seems incorrect: {} USDC",
