...
type SaveToDbFn<K, V> = Arc<dyn Fn(K, V, Duration) -> Pin<Box<dyn Future<Output = ()> + Send>> + Send + Sync>;
type ReadFromDbFn<K, V> = Arc<dyn Fn(&K) -> Pin<Box<dyn Future<Output = Option<V>> + Send>> + Send + Sync>;
    ...
    V: Clone + Send + Sync + Serialize + DeserializeOwned + 'static,
{
    ...
    {
        let loader = Arc::new(loader);
        let read_from_db_arc: ReadFromDbFn<K, V> = Arc::new(move |key: &K| -> Pin<Box<dyn Future<Output = Option<V>> + Send>> {
            Box::pin(read_from_db(key))
        });
        let save_to_db_arc: SaveToDbFn<K, V> = Arc::new(move |key: K, value: V, ttl: Duration| -> Pin<Box<dyn Future<Output = ()> + Send>> {
