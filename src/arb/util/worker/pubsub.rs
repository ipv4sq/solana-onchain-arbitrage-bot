...
impl<T: Send + 'static> PubSubProcessor<T> {
        ...
        F: Fn(T) -> futures::future::BoxFuture<'static, Result<()>> + Send + Sync + 'static + Clone,
    {
                        ...
                        Some(msg) => {
                            if let Err(e) = processor_clone(msg).await {
                                error!(
                                    ...
                                    name_clone, worker_id, e
                                );
                            }
                        }
                        None => {
                            info!(
                                "[{}] Worker {} shutting down - channel closed",
                                name_clone, worker_id
                            );
                            break;
                        }
                    }
        ...
        info!(
            "[{}] Processor initialized with {} workers",
            config.name, config.worker_pool_size
        );
    ...
    pub fn try_publish(&self, message: T) -> Result<()> {
        self.tx_sender.try_send(message).map_err(|e| match e {
