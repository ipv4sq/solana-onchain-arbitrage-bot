...
use crate::convention::chain::instruction::{InnerInstructions, Instruction};
use crate::convention::chain::mapper::traits::ToUnified;
use crate::convention::chain::meta::{TokenBalance, UiTokenAmount};
use crate::convention::chain::{Message, Transaction, TransactionMeta};
...
fn convert_grpc_meta(
    ...
    let loaded_writable_addresses: Vec<Pubkey> = account_keys
        [static_account_len..static_account_len + num_loaded_writable]
        .iter()
        .map(|meta| meta.pubkey)
        .collect();

    let loaded_readonly_addresses: Vec<Pubkey> = account_keys
        [static_account_len + num_loaded_writable..]
        .iter()
        .map(|meta| meta.pubkey)
        .collect();
    ...
    let inner_instructions: Vec<InnerInstructions> = meta
        ...
        .map(|inner| {
            let instructions = inner
                ...
                .enumerate()
                .filter_map(|(idx, ix)| {
                    let program_id = account_keys.get(ix.program_id_index as usize)?.pubkey;
