...
unsafe impl<K: Send, V: Send> Send for MokaCache<K, V> {}
unsafe impl<K: Send, V: Send> Sync for MokaCache<K, V> {}
...
mod tests {
    ...
    async fn test_basic_operations() {
        ...
        let cache = MokaCache::new(3, move |key: &String| {
    ...
    async fn test_capacity_eviction() {
        let cache = MokaCache::new(3, |key: &u32| {
            let key = *key;
    ...
    #[tokio::test]
    async fn test_ttl() {
        let cache = MokaCache::with_ttl(10, Duration::from_millis(100), |key: &u32| {
            let key = *key;
            async move { Some(format!("value_{}", key)) }
        });
    ...
    async fn test_explicit_put() {
        let cache = MokaCache::new(2, |_key: &String| async move {
    ...
    async fn test_invalidation() {
        let cache = MokaCache::new(5, |key: &u32| {
            let key = *key;
            async move { Some(format!("value_{}", key)) }
        });
    ...
    async fn test_concurrent_access() {
        ...
        let cache = Arc::new(MokaCache::new(10, |key: &u32| {
            let key = *key;
            async move {
    ...
    async fn test_multiple_operations() {
        let cache = MokaCache::new(10, |key: &u32| {
            let key = *key;
            async move { Some(format!("value_{}", key)) }
        });
