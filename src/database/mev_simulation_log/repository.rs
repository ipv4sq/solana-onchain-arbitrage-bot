...
impl MevSimulationLogRepository {
    ...
    pub async fn find_by_symbols(
        minor_mint_sym: &str,
        desired_mint_sym: &str,
    ) -> Result<Vec<MevSimulationLog>> {
        let db = get_db().await;
        Ok(MevSimulationLogTable::find()
            .filter(model::Column::MinorMintSym.eq(minor_mint_sym))
            .filter(model::Column::DesiredMintSym.eq(desired_mint_sym))
